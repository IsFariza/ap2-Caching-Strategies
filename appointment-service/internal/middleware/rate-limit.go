...
func (r *RateLimiter) UnaryServerInterceptor() grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		...
		key := "rate_limit:appointment:" + clientIP
		...
		log.Printf(`{"service":"appointment-service","component":"rate_limiter","algorithm":"sliding_window","client_ip":"%s","key":"%s","count":%d,"limit":%d}`, clientIP, key, count, r.limit)
		...
		if count >= r.limit {
			...
			log.Printf(`{"service":"appointment-service","component":"rate_limiter","algorithm":"sliding_window","status":"blocked","client_ip":"%s","retry_after_seconds":%d}`, clientIP, retryAfter)
